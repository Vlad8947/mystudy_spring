...
@Repository. Это уточняющая аннотация по отношению к @Component. Указывает на то, что данный компонент необходим для доступа к данным. Фактически, @Repository является одним из видов аннотации @Component.

@PersistenceContext, в отличии от @Autowired, обладает широким набором атрибутов, которые необходимы для точного указания настроек контекста постоянства.
...
Если функционала методов JpaRepository недостаточно, а описать запрос через название метода проблематично, то можно воспользоваться JPQL (CountryRepository.findArticleByCountry())
...
@Transactional - Чтобы указать контейнеру, что в методе необходимо открыть транзакцию и зафиксировать ее по окончании выполнения метода. Если транзакция подразумевает только чтение из БД, то можно воспользоваться атрибутом readOnly и указать значение true.

----------------
AppConfig

@EnableJpaRepositories – обеспечивает возможность использования Spring Data JPA. В качестве параметра указывается пакет, в котором будут находиться классы-репозитории;
